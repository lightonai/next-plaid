...
./target/release/next-plaid-api -p 3000 -d /data/indices

# Enable debug logging
RUST_LOG=debug ./target/release/next-plaid-api
```
...
./target/release/next-plaid-api --model ./models/colbert

# Enable encoding with CUDA acceleration
./target/release/next-plaid-api --model ./models/colbert --cuda
...
This tutorial walks through building a complete document search system using the Next-Plaid API. We'll create an index with scientific papers, add metadata, and perform various types of searches.

### Prerequisites

- Python 3.8+ with `requests` and `numpy`
- A running Next-Plaid API server (see Quick Start above)
...
```python
#!/usr/bin/env python3
"""Complete Next-Plaid API tutorial example."""
...
kubectl delete pod next-plaid-api-xxxxx
```
...
Next-Plaid uses a three-phase update strategy that explains the varying indexing speeds:
