...
WRITE A MODELS DIRECTORY WITH ONNX SUPPORT. WRITE A CLI TO CONVERT MODELS TO ONNX AND UPLOAD THEM TO THE HUB.
ASSERT THAT ONNX MODEL YIELD EXACT RESULTS AS PYLATE, TO DO SO UPDATE DOCS. THEN FOCUS ON INTEGRATING RUST MODEL INTO THE API. ADD TWO NEW ENDPOINTS OR UPDATE EXISTING ENDPOINT TO BOTH HANDLE TEXT AND EMBEDDINGS. AND WITHIN THOSE METHOD SPECIFY THE MODEL NAME AND THE SEARCH. ADD AN ENCODE ENDPOINT AS WELL WHICH WILL RETURN THE EMBEDDINGS FOR A GIVEN MODEL NAME AND A LIST OF TEXTS.

WRITE A PYTHON SDK TO INTERACT WITH THE RUST API, CONSIDER BOTH SYNC AND ASYNC.

CONSIDER ADDING ON DISK HNSW SUPPORT TO QUERY THE CENTROIDS INSTEAD OF LOADING THEM ALL IN MEMORY.

MAKESURE EVRYTHING CAN BE RUN FROM THE BASE MAKEFILE.

THEN FOCUS ON PYLATE RS FOR EFFICIENT ENCODING ON CPU.
ENABLE ENCODING FROM THE API.
ADD CLIENT TO SEARCH WITHIN TERMINAL.

EXPOSE PARAMETER FOR BUFFER SIZE WHEN CREATING INDICE.

Should we merge queries like we merge documents when updating the index?
