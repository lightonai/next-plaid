...
| Pi          | `colgrep --install-pi`          |

> Restart your agent after installing. Claude Code and Pi have hooks support. OpenCode, Codex, and Hermes integrations are basic for now, PRs welcome.
...
```bash
# CPU
docker pull ghcr.io/lightonai/next-plaid:cpu-1.6.5
docker run -p 8080:8080 -v ~/.local/share/next-plaid:/data/indices \
  ghcr.io/lightonai/next-plaid:cpu-1.6.5 \
...
```bash
# GPU
docker pull ghcr.io/lightonai/next-plaid:cuda-1.6.5
docker run --gpus all -p 8080:8080 -v ~/.local/share/next-plaid:/data/indices \
  ghcr.io/lightonai/next-plaid:cuda-1.6.5 \
  --host 0.0.0.0 --port 8080 --index-dir /data/indices \
