//! Binary for comparing fastkmeans-rs output with Python fastkmeans
//!
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    eprintln!(
        "Running k-means with k={}, seed={}, max_iters={}, tol={}",
    ...
    eprintln!("Centroids shape: {:?}", centroids.shape());

    let writer = File::create(output_path)?;
    centroids.write_npy(writer)?;

    eprintln!("Saved centroids to {}", output_path);
