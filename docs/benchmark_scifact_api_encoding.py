#!/usr/bin/env python3
"""
Benchmark lategrep REST API on SciFact with server-side encoding.
...
2. Connects to an already-running lategrep REST API server with model loaded
...
class LategrepAPIClient:
    """Client for lategrep REST API with encoding support."""
...
def run_api_benchmark(
    ...
    """Run lategrep API benchmark with server-side encoding.
    ...
    try:
        ...
        raise RuntimeError(
            ...
            f"  ./target/release/lategrep-api -h {config.host} -p {config.port} -d ./indices --model <model_path>"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark lategrep API on SciFact with server-side encoding"
    )
    parser.add_argument(
        "--batch-size",
        type=int,
