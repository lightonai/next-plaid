#!/usr/bin/env python3
"""
Benchmark next-plaid REST API on SciFact with incremental updates.

This script:
1. Loads SciFact embeddings (computes and caches if not available)
2. Connects to an already-running next-plaid REST API server
...
def run_api_benchmark(
    ...
    """Run next-plaid API benchmark with incremental updates.
...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark next-plaid API on SciFact with incremental updates"
    )
    parser.add_argument(
    ...
    print("=" * 70)
    print("  SciFact API Benchmark: next-plaid REST API")
    print("=" * 70)
    print("\nConfiguration:")
